...
module.exports = new Event("play", async (client, socket, io, args) => {
    ...
    if(searchResult.playlist) {
        embed = {
            description: `En file d'attente **${searchResult.tracks.length}** tracks from [${searchResult.tracks[0].playlist.title}](${searchResult.tracks[0].playlist.url})`,
            color: 0x44b868
        };
        queue.addTracks(searchResult.tracks);
    } else {
        embed = {
            description: `En file d'attente **[${searchResult.tracks[0].title}](${searchResult.tracks[0].url})**`,
            color: 0x44b868
        };
